...
export async function createFreshTestnetWallets(directory: string) {
  if (process.platform !== "linux" || directory.startsWith("/mnt/"))
    throw new Error("Use protected native Linux storage");
  ...
  if (
    ...
    throw new Error("Unsafe custody directory");
  ...
  await writeFile(
    ...
    JSON.stringify(
      Object.fromEntries(
        wallets.map(({ role, wallet }) => [role, wallet.privateKey]),
      ),
    ),
    { mode: 0o600, flag: "wx" },
  );
  const publicAddresses = Object.fromEntries(
    wallets.map(({ role, wallet }) => [role, wallet.address]),
  ) as Record<(typeof roles)[number], string>;
  await writeFile(
    join(directory, "public-addresses.json"),
    JSON.stringify(publicAddresses, null, 2),
    { mode: 0o600, flag: "wx" },
  );
