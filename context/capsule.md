...
Historical receipt feasibility is verified: `docs/evidence/historical-proof.json` records a third-party Aave repayment authenticated by the actual Creditcoin precompile. This is NOT CureBid-owned execution. ChainInfo maps Sepolia to chain key 1, encoding 1. The real Aave fork passes using faucet LINK collateral; USDC/DAI supplies exceed their configured caps. Public deployment and owned two-chain execution remain funding-blocked. Current delivery details and unresolved acceptance criteria are in `docs/delivery-status.md`; execution is documented in `docs/testnet-runbook.md`. Current user authorization includes commit/push to main, but not fabricated live success or mainnet.
